...
public interface SearchDao extends JpaRepository<Search, Integer>, JpaSpecificationExecutor<Search> {
    ...
    @Query("SELECT s FROM Search s WHERE s.user.userId = :userId AND s.searchType = com.hanbly.ourmusic_api.pojo.Search.SearchType.POSITIVE ORDER BY s.searchTimestamp DESC LIMIT 5")
